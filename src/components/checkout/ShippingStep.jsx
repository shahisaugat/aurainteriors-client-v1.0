...
function AddressSkeletonCard() {
  return (
    <div className="w-full rounded-xl border border-neutral-200 bg-white p-4 animate-pulse">
      <div className="flex items-start gap-3">
        <div className="w-10 h-10 rounded-full bg-neutral-200 shrink-0 mt-0.5" />
...
export default function ShippingStep() {
  ...
  const hydrateAddress = (address) => {
    ...
    setValue("phone", nextAddress.phone, { shouldDirty: false });
    setValue("addressLine1", nextAddress.addressLine1, { shouldDirty: false });
    setValue("addressLine2", nextAddress.addressLine2, { shouldDirty: false });
    setValue("city", nextAddress.city, { shouldDirty: false });
    setValue("state", nextAddress.state, { shouldDirty: false });
    setValue("postalCode", nextAddress.postalCode, { shouldDirty: false });
    setValue("country", nextAddress.country, { shouldDirty: false });
  };
 ...
 useEffect(() => {
  ...
  const handleSaveDrawer = () => {
    if (drawerSelectedAddress) {
      hydrateAddress(drawerSelectedAddress);
    }
    setIsAddressDrawerOpen(false);
  };
  ...
  return (
    <div className="space-y-8 font-dm-sans">
      {isAddressesLoading && <AddressSummarySkeleton />}

      {!isAddressesLoading && addresses.length > 0 && (
        <div className="rounded-xl border border-neutral-200 bg-white overflow-hidden">
          {selectedAddress && (
            <div className="relative w-full text-left px-5 md:px-6 py-5">
              <button
                type="button"
                onClick={handleOpenDrawer}
                className="absolute top-5 right-5 flex items-center gap-2.5 text-[14px] text-neutral-400 hover:text-[#F27318] transition-colors"
              >
              ...
              <div className="flex items-start gap-4">
                <div className="w-11 h-11 rounded-full bg-[#F27318] text-white flex items-center justify-center shrink-0">
                  {(() => {
                    const Icon = addressLabelIcons[selectedAddress.label?.toLowerCase()] || MapPin;
                    return <Icon size={18} />;
                  })()}
                </div>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="mt-3 text-[15px] leading-relaxed text-neutral-600 max-w-4xl">
      ...
      <div className="space-y-4">
        <h3 className="text-[20px] font-semibold text-[#1A1714] font-dm-sans">Delivery Preference</h3>
